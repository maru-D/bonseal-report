const pointsTable = [22, 18, 15, 12, 10, 8, 6, 4, 2, 1];
...
Promise.all([
  ...
  data.races.forEach((race, raceIndex) => {
	...
	if (race.bestDriver && stats[race.bestDriver]) {
  stats[race.bestDriver].bestDriverCount++;
}
  ...
  document.querySelectorAll('#standings .list-group-item').forEach(el => {
    el.addEventListener('click', () => {
      ...
      document.getElementById('driverModalBody').innerHTML = `
		...
		<p><strong>最佳车手次数：</strong>${s.bestDriverCount}</p>
        ...
        <div class="card p-3">
          ...
          <div class="tab-content mt-3">
            <div class="tab-pane fade show active" id="race-tab-${race.round}">
              <p>
  <strong class="${colorClass}">最快圈：</strong> ${race.fastestLap === '/' ? '无最快圈' : race.fastestLap}
  &nbsp;&nbsp;
  <strong class="${colorClass}">最佳车手：</strong> ${race.bestDriver || '未评选'}
</p>
...
document.getElementById('videoBtn').addEventListener('click', () => {
  new bootstrap.Modal(document.getElementById('promoVideoModal')).show();
});
